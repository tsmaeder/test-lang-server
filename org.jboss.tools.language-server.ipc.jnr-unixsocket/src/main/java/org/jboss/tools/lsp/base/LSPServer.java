...
import org.jboss.tools.lsp.transport.jnr.UnixSocketConnection;
...
public class LSPServer implements MessageListener {
	...
	private Connection getConnection(final String stdInName, final String stdOutName) {
		LOGGER.info("Instanciating a new {}", UnixSocketConnection.class.getName());
		return new UnixSocketConnection(stdOutName, stdInName);
	}
	...
	public void send(Message message) {
		connection.send(new TransportMessage(gson.toJson(message)));
	}
	...
	public void messageReceived(TransportMessage message) {
		...
		if (msg == null) {
			LOGGER.warn("unable to parse message {}", message.getContent());
			return;
		}
		LOGGER.info("{} received: {}", msg.getClass().getSimpleName(), message.getContent());
